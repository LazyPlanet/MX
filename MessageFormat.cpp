...
string Message::DebugString() const {
  ...
  MessageFormat::PrintToString(*this, &debug_string);
...
MessageFormat::ParseInfoTree::ParseInfoTree() { }
...
void MessageFormat::ParseInfoTree::RecordLocation(
    const FieldDescriptor* field,
    MessageFormat::ParseLocation location) {
  locations_[field].push_back(location);
}

MessageFormat::ParseInfoTree* MessageFormat::ParseInfoTree::CreateNested(
    const FieldDescriptor* field) {
  // Owned by us in the map.
  MessageFormat::ParseInfoTree* instance = new MessageFormat::ParseInfoTree();
  vector<MessageFormat::ParseInfoTree*>* trees = &nested_[field];
  GOOGLE_CHECK(trees);
  trees->push_back(instance);
  return instance;
}

void AssetCheckFieldIndex(const FieldDescriptor* field, int index) {
...
MessageFormat::ParseLocation MessageFormat::ParseInfoTree::GetLocation(
    const FieldDescriptor* field, int index) const {
	AssetCheckFieldIndex(field, index);
  if (index == -1) { index = 0; }

  const vector<MessageFormat::ParseLocation>* locations =
      FindOrNull(locations_, field);
  if (locations == NULL || index >= (int)locations->size()) {
    return MessageFormat::ParseLocation();
  }

  return (*locations)[index];
}

MessageFormat::ParseInfoTree* MessageFormat::ParseInfoTree::GetTreeForNested(
    const FieldDescriptor* field, int index) const {
	AssetCheckFieldIndex(field, index);
  if (index == -1) { index = 0; }

  const vector<MessageFormat::ParseInfoTree*>* trees = FindOrNull(nested_, field);
...
class MessageFormat::Parser::ParserImpl {
 public:
...
#undef SET_FIELD
  ...
  class ParserErrorCollector : public io::ErrorCollector {
   public:
    explicit ParserErrorCollector(MessageFormat::Parser::ParserImpl* parser) :
        parser_(parser) { }
   ...
   private:
    ...
    MessageFormat::Parser::ParserImpl* parser_;
  };

  io::ErrorCollector* error_collector_;
  MessageFormat::Finder* finder_;
...
class MessageFormat::Printer::TextGenerator {
 public:
...
bool MessageFormat::Parser::Parse(io::ZeroCopyInputStream* input,
                               Message* output) {
  output->Clear();
...
bool MessageFormat::Parser::ParseFromString(const string& input,
                                         Message* output) {
  io::ArrayInputStream input_stream(input.data(), input.size());
  return Parse(&input_stream, output);
}

bool MessageFormat::Parser::Merge(io::ZeroCopyInputStream* input,
                               Message* output) {
  ParserImpl parser(output->GetDescriptor(), input, error_collector_,
                    finder_, parse_info_tree_,
                    ParserImpl::ALLOW_SINGULAR_OVERWRITES,
...
bool MessageFormat::Parser::MergeFromString(const string& input,
                                         Message* output) {
  io::ArrayInputStream input_stream(input.data(), input.size());
  return Merge(&input_stream, output);
}

bool MessageFormat::Parser::MergeUsingImpl(io::ZeroCopyInputStream* /* input */,
                                        Message* output,
...
bool MessageFormat::Parser::ParseFieldValueFromString(
    const string& input,
    const FieldDescriptor* field,
    Message* output) {
  io::ArrayInputStream input_stream(input.data(), input.size());
...
/* static */ bool MessageFormat::Parse(io::ZeroCopyInputStream* input,
                                    Message* output) {
  return Parser().Parse(input, output);
}

/* static */ bool MessageFormat::Merge(io::ZeroCopyInputStream* input,
                                    Message* output) {
  return Parser().Merge(input, output);
}

/* static */ bool MessageFormat::ParseFromString(const string& input,
                                              Message* output) {
  return Parser().ParseFromString(input, output);
}

/* static */ bool MessageFormat::MergeFromString(const string& input,
                                              Message* output) {
...
// to that behavior.
MessageFormat::FieldValuePrinter::FieldValuePrinter() {}
MessageFormat::FieldValuePrinter::~FieldValuePrinter() {}
string MessageFormat::FieldValuePrinter::PrintBool(bool val) const {
  return val ? "true" : "false";
}
string MessageFormat::FieldValuePrinter::PrintInt32(int32 val) const {
  return SimpleItoa(val);
}
string MessageFormat::FieldValuePrinter::PrintUInt32(uint32 val) const {
  return SimpleItoa(val);
}
string MessageFormat::FieldValuePrinter::PrintInt64(int64 val) const {
  return SimpleItoa(val);
}
string MessageFormat::FieldValuePrinter::PrintUInt64(uint64 val) const {
  return SimpleItoa(val);
}
string MessageFormat::FieldValuePrinter::PrintFloat(float val) const {
  return SimpleFtoa(val);
}
string MessageFormat::FieldValuePrinter::PrintDouble(double val) const {
  return SimpleDtoa(val);
}
string MessageFormat::FieldValuePrinter::PrintString(const string& val) const {
  return StrCat("", CEscape(val), "");
  //return StrCat("\"", CEscape(val), "\"");
}
string MessageFormat::FieldValuePrinter::PrintBytes(const string& val) const {
  return PrintString(val);
}
string MessageFormat::FieldValuePrinter::PrintEnum(int32 val,
                                                const string& name) const {
  return name;
}
string MessageFormat::FieldValuePrinter::PrintFieldName(
...
string MessageFormat::FieldValuePrinter::PrintMessageStart(
    const Message& message,
    int field_index,
    int field_count,
...
string MessageFormat::FieldValuePrinter::PrintMessageEnd(
    const Message& message,
    int field_index,
    int field_count,
    bool single_line_mode) const {
...
MessageFormat::Printer::Printer()
  : initial_indent_level_(0),
    ...
    print_message_fields_in_index_order_(true) /*默认打印顺序为定义的索引顺序*/{
...
void MessageFormat::Printer::SetUseUtf8StringEscaping(bool as_utf8) {
  SetDefaultFieldValuePrinter(as_utf8
                              ? new FieldValuePrinterUtf8Escaping()
                              : new FieldValuePrinter());
}

void MessageFormat::Printer::SetDefaultFieldValuePrinter(
    const FieldValuePrinter* printer) {
  default_field_value_printer_.reset(printer);
}

bool MessageFormat::Printer::RegisterFieldValuePrinter(
    const FieldDescriptor* field,
    const FieldValuePrinter* printer) {
  return field != NULL
      && printer != NULL
...
bool MessageFormat::Printer::PrintToString(const Message& message,
                                        string* output) const {
...
bool MessageFormat::Printer::PrintUnknownFieldsToString(
...
bool MessageFormat::Printer::Print(const Message& message,
...
bool MessageFormat::Printer::PrintUnknownFields(
    const UnknownFieldSet& unknown_fields,
    io::ZeroCopyOutputStream* output) const {
  TextGenerator generator(output, initial_indent_level_);
...
void MessageFormat::Printer::Print(const Message& message,
...
void MessageFormat::Printer::PrintFieldValueToString(
    const Message& message,
    const FieldDescriptor* field,
    int index,
    string* output) const {
...
//打印字段，即变量名称
void MessageFormat::Printer::PrintField(const Message& message,
                                     const Reflection* reflection,
                                     const FieldDescriptor* field,
                                     TextGenerator& generator) const {
  ...
  if (field->is_repeated()) {
	...
	if (count == 0) //如果是第一个打印，则初始化下
	{
		std::string name = field->name() + "_count";
		const FieldDescriptor* field_size = message.GetDescriptor()->FindFieldByName(name);
		if (field_size == NULL) return;
		count = message.GetReflection()->GetInt32(message, field_size);
		for (int i = 0; i < count; i++)
		{
			//Message* msg = message.GetReflection()->AddMessage(const_cast<Message*>(&message), field);
		}
	}
...
void MessageFormat::Printer::PrintShortRepeatedField(
    const Message& message,
    const Reflection* reflection,
    const FieldDescriptor* field,
    TextGenerator& generator) const {
...
void MessageFormat::Printer::PrintFieldName(const Message& message,
                                         const Reflection* reflection,
                                         const FieldDescriptor* field,
                                         TextGenerator& generator) const {
...
void MessageFormat::Printer::PrintFieldValue(
    const Message& message,
    const Reflection* reflection,
    const FieldDescriptor* field,
    int index,
...
/* static */ bool MessageFormat::ParseFieldValueFromString(
    const string& input,
    const FieldDescriptor* field,
    Message* message) {
...
void MessageFormat::Printer::PrintUnknownFields(
